...
const { create, stack } = comparatorFactory<LanguageModelProps>();
const comparePreferredLanguageModels = create((a, b) => {
  if (a.priority !== b.priority) {
    if (a.priority === 'preferred') return -1;
    if (b.priority === 'preferred') return 1;
  }
  return 0;
});

export const compareLanguageModels = stack([
  comparePreferredLanguageModels,
  (a, b) => {
