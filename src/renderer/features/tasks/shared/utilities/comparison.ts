...
const compareTasksFactory = (
  ...
  if (b.voteSummary.task[prop] === a.voteSummary.task[prop]) return 0;
  if (b.voteSummary.task[prop] === undefined) return -1;
  if (a.voteSummary.task[prop] === undefined) return 1;
  return b.voteSummary.task[prop] - a.voteSummary.task[prop];
};
...
export const compareProposedTasks: TaskComparisonFunction = (a, b) => {
  ...
  const abstention = b.voteSummary.task.abstained - a.voteSummary.task.abstained;
  ...
  return b.voteSummary.task.awaiting - a.voteSummary.task.awaiting;
};
...
export const compareAwaitingTasks: TaskComparisonFunction = (a, b) => {
  const awaiting = b.voteSummary.task.awaiting - a.voteSummary.task.awaiting;
