...
import { interpolateHue } from "@/shared/utilities";
import {
...
export const TaskScoreCellRenderer: CellRenderer = ({ row: { voteSummary: { task } } }) => {
  const { current } = useNavigation();
  // TODO: This shouldn't happen (at least for long) so I guess display an error.
  // TODO: Is this information not available and handled better in the task context?
  if (!current) return null;

  const view = current.name as TaskListViewConfigName;
  const [hero, subscore, badge] = viewVoteScores[view];

  return <HeroSubscore
    badge={badge && task[badge]}
    hero={task[hero]}
    subscore={subscore && task[subscore]}
    type={hero}
  />;
};
