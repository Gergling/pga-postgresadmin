...
console.log('status cell renderer')

const statusConfig = {
  proposed: { label: 'Proposed', color: COLORS.goldGlow, icon: AutoFixHigh },
  todo: { label: 'To Do', color: '#2196F3', icon: Circle },
  doing: { label: 'Doing', color: COLORS.ruddy, icon: Pending },
...
export const TaskStatusCellRenderer: CellRenderer = ({ row }) => {
  const { envelope: { data: { status } }, view } = row;
