...
export const taskImportanceSchema = z.enum([
  ...TASK_VOTE_BASE, ...TASK_IMPORTANCE
].map(({ name }) => name));
...
export const taskMomentumSchema = z.enum([
  ...TASK_VOTE_BASE, ...TASK_MOMENTUM
].map(({ name }) => name));
export type TaskMomentum = z.infer<typeof taskMomentumSchema>;

export const COUNCIL_MEMBER = [
  { name: 'librarian', label: 'The Librarian', color: '#607D8B' }, // Slate
  { name: 'sceptic', label: 'The Sceptic', color: '#FF5252' }, // Red
  { name: 'guardian', label: 'The Guardian', color: '#4CAF50' }, // Green
  { name: 'strategist', label: 'The Strategist', color: '#2196F3' }, // Blue
  { name: 'philosopher', label: 'The Philosopher', color: '#9C27B0' }, // Purple
  { name: 'diplomat', label: 'The Diplomat', color: '#E91E63' }, // Pink
  { name: 'architect', label: 'The Architect', color: '#FF9800' }, // Orange
] as const;
export const councilMemberNamesSchema = z.enum(COUNCIL_MEMBER.map(({ name }) => name));
export type CouncilMemberNames = z.infer<typeof councilMemberNamesSchema>;
